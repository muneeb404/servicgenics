...
contactForm.addEventListener("submit", function (e) {
  ...
  const email = document.getElementById("email").value.trim();
  const message = document.getElementById("message").value.trim();
  const option = document.querySelector('input[name="options"]:checked')?.value || "No Option";
  ...
  fetch("https://api.web3forms.com/submit", {
    ...
    .then(result => {
      if (result.success) {
        ...
        msgError.innerText = "Something went wrong. Please try again.";
      }
    })
    .catch(() => {
      msgError.innerText = "Network error. Please try again.";
    });
});
...
newsletterForm.addEventListener("submit", function (e) {
  ...
  const isEmailValid = showError(email, newsletterError, "Valid email is required", validateEmail);
  ...
  fetch("https://api.web3forms.com/submit", {
    ...
    .then(result => {
      if (result.success) {
        ...
        newsletterError.innerText = "Something went wrong. Please try again.";
      }
    })
    .catch(() => {
      newsletterError.innerText = "Network error. Please try again.";
    });
});
